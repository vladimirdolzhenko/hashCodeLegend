...
что такое ассоциативный массив ? это тип данных, позволяющий хранить пары ключ-значение и поддерживающий операции вставки, поиска и удаления по ключу.
...
проблема заключена в том, что hashmap всё равно пытается строить деверо - для сравнения, используется системный hashcode - и рекурсивный, а не итеративный поиск внутри дерева.
...
Увы, но эти рассуждения не совместимы с сборщиками мусора, используемыми в обычной жизни - и уже из предыдущего опыта мы видели, что объекты создаются в eden, который в нашем случае оказался равен 64Мб - и в этом случае нам удастся создать только 4млн объектов, после чего начнутся коллизии.
...
Но, потенциально может и OOM произойти - кто за OOM ? Но самый главный вопрос этого эксперимента - как будут распределены адреса ?

Мы же ожидаем, что eden равномерно делится несколькими нитями - каждый шаг мы создаём по одному объекту - а phaser даёт нам ту самую гарантию, что ни одна нить не убежит вперёд других, больше, чем на один наг.

Какие наши ожидания ? Что сможем создать столько объектов всего - т.е во всех нитях - что и в однопотомчном - и разница между адресами будет составлять 16 байт - т.е размер объекта - может быть +- 1.

Кто согласен с этим ? Может есть кто не согласен и увидим другое ?
...
Как и ожидали - случились коллизии, а не OOM - Вот чего не ожидали, так это такой большой разницы?
...
Так же, как и моделируемом аллокаторе я использовал 4 нити - и заметно насколько аллокакция с TLAB лучше, чем без неё - почти в 100 раз
