...
Ассоциативный массив - тип данных, позволяющий хранить пары ключ-значение и поддерживающий операции вставки, поиска и удаления по ключу.
...
Важный момент - поскольку число может быть любым в пределах 32х бит - т.е как очень большим - так и отрицательным, а доступная память ограничена - поэтому полученное число предстоит ещё отмасштабировать на длину массива.
...
hashcode это не всякая функция, которая ставит в соответствии с объектом какое-то число.

одно из ключевых свойств hashcode - это не изменчивость и постоянство - и javadoc определяет контракт для hashcode -  hashCode должен постоянно возвращать одно и то же значение.

# Нарушение контракта hashCode

т.е hashCode Алексея по какой-либо причине изменится - то мы просто не найдём Алексея.
...
И один из отцов Java - небезызвестный Джошуа Блох решил это исправить
