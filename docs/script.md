...
нам помогут такие структуры данных как бинарные деревья, которые обладают логарифической сложностью.

В java начиная с 8ки - внутри HashMap, LinkedHashMap, ConcurrentHashMap используется красно-чёрное дерево для разрешения коллизияй, привышении некоторого порогового числа коллизий - пока уровень коллизий мал, а именно меньше 8 элементов, то там по прежнему chaining.
...
основной эффект прежде всего из-за изменения внутренней реализации hashmap - если сделал backport hashmap из 8ки в 7ку - то цифры будут схожими с теми, что мы видим для 8ки.
...
И для наглядности - распределением hashcode по нитям, но с выключенным TLAB
