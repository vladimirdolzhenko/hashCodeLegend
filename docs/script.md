...
Прежде всего для построения ассоцитивного массива нужна функция, ставящая в соответствии с ключом некоторое числовое значение.

у java.lang.Object - корневого класса всей иерархии классов в java - определен метод hashcode - т.е по сути каждый объект в java обладают некоторой стандартной hash-функцией.
...
Идея заключается в том, что используя hashCode можно сразу вычистлить индекс в массиве, куда записать ключ и соответственно значение.

Важный момент - поскольку число может быть любым в пределах 32х бит - т.е как очень большим - так и отрицательным, а доступная память ограничена - поэтому полученное число предстоит ещё отмасштабировать на длину массива - например, можно использовать остаток от деления на длину массива.
...
И тогда мы будем разрешать коллизии с помощью гибрида из hash структуры и бинарного дерева поиска - в java начиная с 8ки - внутри HashMap, LinkedHashMap, ConcurrentHashMap используется красно-чёрное дерево при привышении некоторого порогового числа коллизий - пока уровень коллизий мал, то там по прежнему chaining.
...
В демонстрационных целях - чтобы не ждать OOM долго - я буду ограничивать размер кучи 256Мб.
