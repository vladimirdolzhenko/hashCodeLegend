...
Меня зовут Владимир и мы будем сквозь замочную скважину заглядывать внутрь VM.
...
* и затем окунёмся в потроха виртуальной машины при помощи всё того же hashcode - будет и сборщик мусора, и аллокация, и немного баттлов
...
В этом заключается главная мысть использования hashCode.
...
Теперь ложка дёгтя - что случится, если hashCode Фёдора по какой-либо причине изменится - если теперь мы попробуем найти его в нашей телефонной книге - то мы просто не найдём Фёдора.
...
# Chaining
И т.к такой идеальной функции нет - то используют то, что есть - а есть то, что либо вернул метод hashCode() - и что там пользователь напишет - может быть ужас-ужас. И разрешают коллизии - один из способов её разрешить - chaining - это создать список и поместить все ключи с одинаковыми hashcode в него. способ универсальный, но далек от идеала, и как плата за это - сложность поиска снова становится линейной. Это очень важный момент - коллизии способны сломать производительность, и чем больше коллизий, тем только хуже операции типа поиска, вставки и т.п
...
Т.к. метод hashcode определён у java.lang.Object - значит все классы, которые не переопределяют этот метод - ведут себя точно так же. И что же собственно он возвращает ?
...
И первый эксперимент - проверить насколько можно доверять адресу объекта - для проверки этой гипотезы давайте создадим новый объект - theObject - в данном демо это будет главная цель нашего наблюдения - зафиксируем адрес этого объекта в самом начале, затем будем создавать другие объекты и, чтобы избежать того, чтобы GC собрал их как мусор - будем хранить их в списке gcKeeper.
...
Теперь видно, что произошла сборка мусора - и более того, можно даже понять откуда и куда был перемещён наш объект.
...
и jmh это фреймфорк по написанию правильных nano/micro/milli performance benchmarks для java. он как и java object layout составляет часть openjdk toolset и так же люди из openjdk его поддерживают и развивают.

# Java Microbenchmark Harness

Примерно вот так или подобно выглядит benchmark на jmh - у нас есть само тело бенчмарка - как раз мы измеряем как быстро мы можем аллоцировать память - jmh даёт возможность защитится от множества скрытых подводных камней, связанных с работой VM - н-р если VM понимает, что результат выполнения никем и никогда не используется - так зачем и сам блок выполнения нужен - он может быть убран - и blackhole спасает от этого (хотя более простой способ - это просто вернуть объект или число из бенчмарк-метода - но оно всё равно попадёт в blackhole)

jmh на себя берёт вопросы прогрева vm - как долго надо разогревать, как долго надо измерять сам бенчмарк, чтобы получить статистически корректные измерения, можно специфицировать в скольких нитях будет работать бенчмарк,
по аналогии с junit можно специфицировать setup и teardown методы, которые соот-но вываваются раз перед и после работы тела бенчмарка, и многие другие полезные вещи.

конечно - это не избавляет от того, чтобы писать неправильные benchmark-и и/или неправильно их трактовать - если ещё не делали - смотрите записи Алексея Шипилёва, где он подробно расскаывает о jmh, на сайте проекта есть подробное примеры.

словом - jmh - важный и полезный инструмент в руках инженера.

# SyncAllocator Performance Benchmark

такой вот простой бенчмарк, чтобы измерить производительность нашего аллокатора - за кадром осталось, что это на 10 потоках - в реальных приложениях их намного больше - и такой же самый бенчмарк был сделан, но для однопоточного аллокатора
...
Давайте соорудим похожий эксперимент, только работающее в нескольких нитях - и будем работать до первой коллизии - не важно, в какой из нитей она бы не произошла.
...
Для практических задач куда более интерен случай, когда хэш-функция это некоторая функция над содержимым объекта.

И хорошо бы знать, как его можно он должен быть как-то вычислен - ведь не важно - получили ли мы это имя по сети, или пользователь ввёл в форму данные, или как-то ещё - ключ один и тот же с точки зрения equals - а следовательно и hashcode должен быть один и тот же.

Т.е, что строки java, что экземпляры 42 - каждый из них имеет один и тот же hashcode соответственно.

Какие же правила для вычисления hashcode ?
...
И тогда мы будем разрешать коллизии с помощью такого гибрида из hash структуры и бинарного дерева поиска - в java начиная с 8ки - внутри HashMap, LinkedHashMap, ConcurrentHashMap используется красно-чёрное дерево при привышении некоторого порогового числа коллизий. Если уровень коллизий мал, то там по прежнему chaining.
