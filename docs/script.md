...
Другой способ разрешения коллизий - Открытая адресация - заключается в том, что делается последовательность проб в массиве ключей относительно исходного индекса - если ячейка занята - делают следующую пробу - например, выбирают соседнюю ячейку
...
Одна из вариаций разрешения коллизий заключется в использовании двойного хэширования.

Известно, что SHA1 с очень маленькой вероятностью, но всё же обладает коллизиями, у MD5 вероятность коллизий выше - но поскольку базис данных hash-функций разный - то и результирующая вероятность коллизии почти, что нулевая.

и почему бы не взять ещё одну функцию - и желательно, ортогональной исходной - как это, например, может быть вторая hash-функция, либо другая функция

# Дополнительная функция: compareTo

можно рассматривать compareTo как такую вторую функцию - ортогональную к hash-функции
и тем более, что и строки, и числа, даты и многие другие классы в стандартной библиотеке уже сравниваемые - т.е у них реализована функция compareTo

и тогда для разрешения коллизий возьмём такие структуры данных как бинарные деревья, которые обладают логарифической сложностью.
