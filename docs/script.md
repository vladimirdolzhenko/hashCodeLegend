...
* и как можно используя знания о hashcode устроить DDOS атаку, и как вылечить её
...
Найти человека из нашей телефонной книги, например Алексея, но Фёдорова
...
Это Лёша, но другой - опять идём дальше
...
Из предыдущего демо стало ясно, что коллизии вызваны размером eden - интересно как себя будет вести тот же сценарий, но в условиях многопоточности - как всегда ожидания наши - либо OOM, либо коллизии, но уж наверняка-то вывалятся коллизии.

Кто за OOM ? А кто за коллизии ?

(показываем код **ThreadedIdentityHashCodeCollision** - здесь различий куда больше между слайдом и реальным кодом - здесь больше инфраструктуры демо - это CountDownLatch позволяющие добится того, что потоки будут запущены и работать параллельно, а не так, чтобы какой-то уже начал работать сильно раньше других - и количество потоков получаю как аргумент для запуска приложения - запускаем демо **ThreadedIdentityHashCodeCollision 4** на 4х нитях - и чтобы была многопоточность, и чтобы для демо не слишком много)

Как и ожидали - случились коллизии

# Всё ли в порядке ?
Если посмотрим - да коллизии случились, получается несколько нитей совместно делят eden - но в сумме было столько же создано объектов, как и в однопоточном случае - пока никаких аномалий, единственное что настораживает - где-то быстрее, где-то медленее.

# Почему такие коллизии ?

Но почему именно такие коллизии - какой-то неожиданно большой разброс между коллизями в разных нитях. Поскольку мы имеем дело с адресами, и созданием объектов, то нам надо рассматривать очень внимательно и вопрос выделения памяти
...
при 10 параллельных нитях - почти 400нс на одну аллокацию - это очень дорогая аллокация - для сравнения simple allocator, который применим только к однопоточному случаю, почти в 100 раз дешевле
...
Так же 10 нитей - стало лучше, но не сильно лучше - 245 нс на операцию очень дорого...
...
Подсказку нам может дать сама java - для этого создадим в каждом потоке по одному объекту, и посмотрим на распределение hashcode-ов этих объектов

(показываем код **SingleObjectThreadedHashCodeNoTLAB** - так же немного инфраструктуры для демо, чтобы нити не убегали одна вперёд другой - и весь вывод делать будем вне нитей - чтобы избежать паразитных аллоакций - запускаем демо **SingleObjectThreadedHashCodeNoTLAB 4**)

# Распределение hashCode по нитям - результаты

куда более наглядно и познавательно выглядит разница между hashcode объектов - т.е между адресами объектов в разных нитях - и эта разница больше 1 Мб.
...
Круто - вот это уже выглядит намного лучше.
...
# Распределение адресов по потокам БЕЗ TLAB

И теперь запустим то же демо **SingleObjectThreadedHashCodeNoTLAB 4 -TLAB**, которое создаёт по одному объекту, и снова жанглируя с флажками JVM - на этот раз отключив TLAB
...
Итак, использование GC и идеи поколений, быстрой аллокации и TLAB приводят к тому, что hashCode из 32х бит потерял почти половину. Коллизии начинают встречаться уже через какие-то два-три миллиона новых объектов, после чего все значения строго повторяются.
...
И вот как выглядит сравнение того же самого теста с коллизями по username в 7ке и в 8ке - вот тот график, который слабо отличается от нуля - это 8ка - ибо там совершенно другой масштаб времени - положить все 200 тыс ключей на 8ке занимает где-то 85 мс

Кто ещё свои приложения в проде держит на 7ке ?  или на 6ке ?)

# "username" коллизии :: java8

И вот результаты работы тех же 200 тыс коллизий на 8ке в пересчёте на одну вставку - прежде всего обратите внимание на шкалу времени - это наносекунды - и поведение графика чётко указывает на логарифическую сложность
